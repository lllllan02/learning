...
Go 虽然是并发 [[golang/gc/guide|GC]]，但在每个周期的开始和结束，都有两个必须 **Stop-The-World (STW)** 的阶段。这是程序在理论上“绝对”会停顿的时刻。

1.  **标记准备阶段 (Sweep Termination)**
    *   动作：清理上一轮 GC 剩余的任务，开启[[golang/gc/write_barrier|写屏障（Write Barrier）]]。
...
1.  **抢占延迟 (Preemption Delay)**
    *   原因：在进入 STW 前，GC 必须等待所有 Goroutine 停在[[golang/gc/safe_point|安全点]]。如果某个 Goroutine 处于无法被抢占的状态（如紧凑循环、长时间 CGO 调用），会导致整个程序停下来等它一个。
...
1.  **[[golang/gc/mark_assist|GC 协助标记]] (GC Assist / Mark Assist)**
